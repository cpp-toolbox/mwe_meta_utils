...
class MetaProgram {
  ...
  public:
    ...
    std::string int_to_string(int v) { return std::to_string(v); }
    int string_to_int(std::string s) { return std::stoi(s); }
    ...
    std::string unsigned_int_to_string(unsigned int v) { return std::to_string(v); }
    int string_to_unsigned_int(std::string s) { return static_cast<unsigned int>(std::stoul(s)); }
    ...
    std::string uint8_t_to_string(uint8_t v) { return std::to_string(v); }
    uint8_t string_to_uint8_t(std::string s) { return static_cast<uint8_t>(std::stoul(s)); }
    ...
    std::string float_to_string(float v) { return std::to_string(v); }
    float string_to_float(std::string s) { return std::stof(s); }
    ...
    std::string double_to_string(double v) { return std::to_string(v); }
    double string_to_double(std::string s) { return std::stod(s); }
    ...
    std::string short_to_string(short v) { return std::to_string(v); }
    short string_to_short(std::string s) { return static_cast<short>(std::stoi(s)); }
    ...
    std::string long_to_string(long v) { return std::to_string(v); }
    long string_to_long(std::string s) { return std::stol(s); }
    ...
    std::string std__string_to_string(std::string s) { return s; }
    std::string string_to_std__string(std::string s) { return s; }
    ...
    std::string deserialize_std__string(std::vector<uint8_t> buf) {
        ...
        if (buf.size() < sizeof(size_t) + len)
            return std::string();
        return std::string(reinterpret_cast<const char *>(buf.data() + sizeof(size_t)), len);
    }
    std::string bool_to_string(bool v) { return std::to_string(v); }
    bool string_to_bool(std::string s) { return s == "true" || s == "1"; }
    std::vector<uint8_t> serialize_bool(bool v) {
        std::vector<uint8_t> buf(1);
        buf[0] = v ? 1 : 0;
        return buf;
    }
    bool deserialize_bool(std::vector<uint8_t> buf) { return buf[0] != 0; }
    std::string meta_utils__MetaType_to_string() { return ""; }
    ...
    std::vector<uint8_t> serialize_B(B obj) {
        ...
        }
        {
            auto ser = [=](const std::array<float, 3> &arr) -> std::vector<uint8_t> {
                std::vector<uint8_t> buffer;
                auto element_serializer = [](const float &v) {
                    std::vector<uint8_t> buf(sizeof(float));
                    std::memcpy(buf.data(), &v, sizeof(float));
                    return buf;
                };
    ...
    B deserialize_B(std::vector<uint8_t> buffer) {
        ...
        size_t offset = 0;
        {
            auto deser = [](const std::vector<uint8_t> &buf) {
                int v;
                std::memcpy(&v, buf.data(), sizeof(int));
                return v;
            };
        ...
        }
        {
            auto deser = [=](const std::vector<uint8_t> &buffer) -> std::array<float, 3> {
                std::array<float, 3> result{};
                auto element_deserializer = [](const std::vector<uint8_t> &buf) {
                    float v;
                    std::memcpy(&v, buf.data(), sizeof(float));
                    return v;
                };
            ...
            size_t len = sizeof(obj.position);
            if (offset + len > buffer.size())
                return obj;
            std::vector<uint8_t> slice(buffer.begin() + offset, buffer.begin() + offset + len);
    ...
    std::vector<uint8_t> serialize_X(X obj) {
        ...
        }
        {
            auto ser = [=](const std::vector<int> &vec) -> std::vector<uint8_t> {
                ...
                auto element_serializer = [](const int &v) {
                    std::vector<uint8_t> buf(sizeof(int));
                    std::memcpy(buf.data(), &v, sizeof(int));
                    return buf;
                };
    ...
    X deserialize_X(std::vector<uint8_t> buffer) {
        ...
        }
        {
            auto deser = [=](const std::vector<uint8_t> &buffer) -> std::vector<int> {
                ...
                size_t elem_size = sizeof(int);
                if (offset + count * elem_size > buffer.size())
                                ...
                                if (std::getline(iss, token, ',')) {
                                    ...
                                    if (pos != std::string::npos) {
                                        ...
                                        auto conv = [=](const std::string &input) -> std::vector<int> {
                                            ...
                                            std::regex element_re(R"(-?\d+)");
                                            auto begin =
                                                std::sregex_iterator(trimmed.begin(), trimmed.end(), element_re);
                                            auto end = std::sregex_iterator();
                                            ...
                                            if (std::getline(iss, token, ',')) {
                                                ...
                                                if (pos != std::string::npos) {
                                                    ...
                                                    auto conv = [=](const std::string &input) -> std::array<float, 3> {
                                                        ...
                                                        std::array<float, 3> result{};
                                                        std::regex element_re(R"(-?\d+(?:\.\d+)?)");
                                                        auto begin = std::sregex_iterator(trimmed.begin(),
                                                                                          trimmed.end(), element_re);
                                                        auto end = std::sregex_iterator();

                                                        size_t idx = 0;
                                                        for (auto it = begin; it != end && idx < 3; ++it, ++idx) {
                                                            try {
                                                                auto conversion = [](const std::string &s) {
                                                                    return std::stof(s);
                                                                };
                                                                result[idx] = conversion(it->str());
                                                            } catch (...) {
        ...
        }
        {
            auto deser = [=](const std::vector<uint8_t> &buffer) -> std::vector<X> {
                    ...
                    }
                    {
                        auto deser = [=](const std::vector<uint8_t> &buffer) -> std::vector<int> {
                            ...
                            if (offset + count * elem_size > buffer.size())
                                return result; // safety check
                            for (size_t i = 0; i < count; ++i) {
                                std::vector<uint8_t> elem_buf(buffer.begin() + offset,
                                                              buffer.begin() + offset + elem_size);
                                int elem = element_deserializer(elem_buf);
                                result.push_back(elem);
                                offset += elem_size;
                            }
                        ...
                        auto deser = [=](const std::vector<uint8_t> &buffer) -> B {
                            ...
                            }
                            {
                                auto deser = [](const std::vector<uint8_t> &buf) {
                                    ...
                                    if (buf.size() < sizeof(size_t) + len)
                                        return std::string();
                                    return std::string(reinterpret_cast<const char *>(buf.data() + sizeof(size_t)),
                                                       len);
                                };
                                if (offset + sizeof(size_t) > buffer.size())
                                    return obj;
                ...
                for (size_t i = 0; i < count; ++i) {
                    ...
                    offset += sizeof(size_t);
                    if (offset + elem_size > buffer.size())
                        break;
                    std::vector<uint8_t> elem_buf(buffer.begin() + offset, buffer.begin() + offset + elem_size);
